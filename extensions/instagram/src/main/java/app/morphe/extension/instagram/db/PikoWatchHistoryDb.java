...
public class PikoWatchHistoryDb extends SQLiteOpenHelper {
    ...
    /** Prefetch metadata is a cache, not history, so it gets a much smaller cap of its own. */
    private static final int MAX_UNWATCHED_ROWS = 300;
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (oldVersion < 3) {
            // Every existing row was written by a prefetch-time binder, so none of them is a
            // confirmed watch. Demoting them all is what clears the unseen media out of the list.
            db.execSQL("ALTER TABLE " + TABLE + " ADD COLUMN watched INTEGER NOT NULL DEFAULT 0");
        }
    }

    /**
     * Records a watch, or refreshes cached metadata without claiming the media was watched.
     *
     * <p>Prefetch-time callers pass {@code watched = false}: Instagram builds state for the media
     * on screen and the next few queued behind it at the same moment, so those rows exist only to
     * carry username and cover art. They stay out of {@link #query} until real playback promotes
     * them, and they never move an existing row to the top of the list.
     */
    public void upsert(Entry entry, boolean watched) {
        if (entry == null || entry.mediaId == null || entry.mediaId.isEmpty()) return;
        SQLiteDatabase db = getWritableDatabase();
        Cursor existing = db.rawQuery(
            "SELECT type, username, title, caption, hashtags, permalink, watched_at, cover_url, watched FROM "
                + TABLE + " WHERE media_id = ?",
            new String[]{entry.mediaId}
        );
        try {
            if (existing.moveToFirst()) {
                boolean alreadyWatched = existing.getInt(8) != 0;
                ...
                values.put("watched", alreadyWatched || watched ? 1 : 0);
                values.put("watched_at", watched ? entry.watchedAt : existing.getLong(6));
    ...
    /** Each tier is capped separately so cached prefetch rows can never evict real history. */
    private void trimToCap(SQLiteDatabase db) {
        db.execSQL(
            "DELETE FROM " + TABLE + " WHERE watched = 1 AND rowid NOT IN (" +
            "SELECT rowid FROM " + TABLE + " WHERE watched = 1 ORDER BY watched_at DESC LIMIT "
            + MAX_ROWS + ")"
        );
        db.execSQL(
            "DELETE FROM " + TABLE + " WHERE watched = 0 AND rowid NOT IN (" +
            "SELECT rowid FROM " + TABLE + " WHERE watched = 0 ORDER BY rowid DESC LIMIT "
            + MAX_UNWATCHED_ROWS + ")"
        );
    }
